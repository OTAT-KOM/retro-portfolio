...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Handle Media Source Upload
    if (isset($_FILES['media_src']) && $_FILES['media_src']['error'] === UPLOAD_ERR_OK) {
        $uploadedPath = handleUpload($_FILES['media_src']);
        if ($uploadedPath) {
            $item['media_src'] = $uploadedPath;
        }
    }

    $item['id'] = $_POST['id'] ?? uniqid('proj');
    $item['title'] = $_POST['title'] ?? '';
    $item['type'] = $_POST['type'] ?? 'small';
    $item['media_type'] = $_POST['media_type'] ?? 'image';
    ...
    if (isset($_POST['gallery_index'])) {
        $indexes = $_POST['gallery_index'];
        foreach ($indexes as $i) {
            if (isset($_POST['delete_gallery'][$i]) && $_POST['delete_gallery'][$i] == '1') continue;
...
<html>
<head>
    <title><?php echo $isNew ? 'Add Project' : 'Edit Project'; ?> - OTAT Admin</title>
    <style>
        ...
        function addGalleryItem() {
            ...
            const html = `
                <div class="gallery-item" id="gallery-item-${index}">
                    ...
                    <div>
                        <button type="button" class="btn-remove" onclick="document.getElementById('gallery-item-${index}').remove()">Remove</button>
                    </div>
                </div>
            `;
...
<body>
    <div class="container">
        <a href="edit_projects.php" class="back-link">&larr; Back to List</a>
        <h2><?php echo $isNew ? 'Add Project' : 'Edit Project'; ?></h2>
        
        <form method="post" enctype="multipart/form-data">
            <input type="hidden" name="id" value="<?php echo htmlspecialchars($item['id'] ?? uniqid('proj')); ?>">
            ...
            <div class="form-group">
                <label>Media Source (Main)</label>
                <?php if (!empty($item['media_src'])): ?>
                    <?php if (($item['media_type'] ?? 'image') == 'video'): ?>
                        <video src="../<?php echo $item['media_src']; ?>" class="current-image" controls></video><br>
                    <?php else: ?>
                        <img src="../<?php echo $item['media_src']; ?>" class="current-image"><br>
                    <?php endif; ?>
                    <small>Upload new to replace</small>
                <?php endif; ?>
                <input type="file" name="media_src">
            </div>
                ...
                ?>
                    <div class="gallery-item" id="gallery-item-<?php echo $uniqueIdx; ?>">
                        ...
                        <?php if (!empty($gItem['src'])): ?>
                            <?php if (($gItem['type'] ?? 'image') == 'video'): ?>
                                <video src="../<?php echo $gItem['src']; ?>" class="gallery-preview"></video>
                            <?php else: ?>
                        ...
                        <div>
                            <button type="button" class="btn-remove" onclick="document.getElementById('gallery-item-<?php echo $uniqueIdx; ?>').remove()">Remove</button>
                        </div>
                    </div>
                <?php endforeach; ?>
            </div>
